...
public class NbpMetaProgramInitHelpers {
    ...
    public static void copy_xsd(SdmDataObject obj) {
        try {
            ...
            file_content = NbpHelpers.read_from_jar_file(Const.SDM_XSD);
            NbpHelpers.save_text_to_file(obj, output_dir_rel_path, Const.SDM_XSD, file_content);
    ...
    public static void copy_dto_xml(SdmDataObject obj) {
        try {
            String output_dir_rel_path = NbpPathHelpers.get_folder_relative_path(obj);
            String file_content = NbpHelpers.read_from_jar_file(Const.SDM_XML);
            NbpHelpers.save_text_to_file(obj, output_dir_rel_path, Const.SDM_XML, file_content);
        } catch (Exception ex) {
            // ex.printStackTrace();
            NbpIdeMessageHelpers.show_error_in_ui_thread(ex);
        }
