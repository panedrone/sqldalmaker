...
static void getFieldNames(sqlite3_stmt *pStmt, CSimpleArray<CStringW> &res) {
	...
	for (int i = 0; i < mnCols; i++) {
		...
		DL_STR szTemp = (DL_STR) sqlite3_column_name16(pStmt,
				i);
...
static int getFieldIndex2(CSimpleArray<CStringW> &field_names,
		DL_STR name) {
	...
	for (int col = 0; col < field_indexes.GetSize(); col++) {
		...
		if (r == SQLITE_NULL) {
			...
			case VALUE_TYPE_LONG:
				...
				((CLong *) ret_values[i])->Value = sqlite3_column_int64(pStmt,
						i);
				break;
			case VALUE_TYPE_DOUBLE:
...
int CDataStore::queryRow(DL_STR sql, int params_count,
	...
	if (nRet != SQLITE_OK) {
		...
		if (nRet == SQLITE_DONE) {
			...
			getFieldIndexes(field_names, ret_values_count, col_names,
					field_indexes);

			if (readRowData(pStmt, ret_values_count, ret_values,
					field_indexes)) {
				num_rows = 1;
			}
		}
